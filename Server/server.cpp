...
void handleClient(int client_socket) {
    ...
    while (!handleLoginRequest(client_socket, message_sequence, username_sum, password_sum)) {
        std::cout << "Login failed. Client socket: " << client_socket << std::endl;
        close(client_socket);
        return;
    }

    // Handle echo requests until "exit" is received
    while (handleEchoRequest(client_socket, message_sequence, username_sum, password_sum)) {
        // Continue handling echo requests
    }
